...
class ParallelCountersignIT {
    ...
    void allThreeMustApproveBeforeProceed() throws IOException {
        String raw = new String(
                getClass().getResourceAsStream("/bpm/it/parallel-countersign.bpmn20.xml").readAllBytes(),
        ...
        // Complete 2 — instance still active
        taskService.complete(tasks.get(0).getId());
        taskService.complete(tasks.get(1).getId());
        assertThat(runtimeService.createProcessInstanceQuery().processInstanceId(inst.getId()).singleResult()).isNotNull();

        // Complete 3rd — instance ends
